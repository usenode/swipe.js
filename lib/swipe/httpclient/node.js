...
pkg.define('swipe_httpclient_node', ['node:sys', 'promise', 'node:http', 'node:url'], function (sys, promise, http, url) {
    ...
    var Client = ns.Client = function (instrument) {
        ...
        this.setProxyFromEnv();
    };

    Client.prototype.setProxyFromEnv = function () {
        var envProxy = process.env.HTTP_PROXY || process.env.http_proxy;
        if (typeof envProxy === 'undefined') {
            return;
        }
        var match = envProxy.match(/^http:\/\/(.+):(\d+)\/?$/);
        if (! match) {
            throw new Error('unrecognised http proxy format: "' + envProxy + '"');
        }
        this.setHttpProxy(match[1], parseInt(match[2]));
        var noProxy = process.env.NO_PROXY || process.env.no_proxy;
        if (typeof noProxy === 'undefined') {
            return;
        }
        var hosts = noProxy.split(/\s*\,\s*/);
        for (var i = 0, l = hosts.length; i < l; i++) {
            this.addProxyException(hosts[i]);
        }
    }
        ...
        request.addListener('response', function (nodeResponse) {
            ...
            nodeResponse.addListener('end', function () {
                ...
                if (response.isSuccess()) {
                    ...
                    // TODO make sure responses stringify well for good error messages
